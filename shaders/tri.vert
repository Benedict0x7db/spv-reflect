...
void main()
{
    vec4 pos0 = dd_MVP.MVP * vec4(position.xy, 0.0, 1.0);
    vec4 pos1 = dd_MVP.MVP * vec4(position.xy + dirDis.xy * dd_half_width.half_width , 0.0, 1.0);
    umf_df_adf.z = position.z + dirDis.z * dd_offset.offset;
    gl_Position = pos1;
    if (dirDis.z == 0.0)
        gl_Position = pos0;
    ...
    p1 -= p0;
    p1.y *= dd_h_w.h_w;
    float ratio = length(p1) * dd_halfPixelWidth_rev.halfPixelWidth_rev;
    ratio = min(ratio, 1.3);
    umf_df_adf.x = dd_unit_max.unit_max * ratio;
